...
sweep (edits landed, Ford has not read the doc yet). "title strip"
(R2-D11, 2026-07-17) = leading duplicate-of-folder-name H1 removed when
the new fixed page title furniture landed; docs WITHOUT the note kept a
richer opening H1 that now renders under the page title — flag during
read-through if unwanted.

| Doc | Reviewed | Verdict | Actions |
|---|---|---|---|
| docs/00-foundation/00-manifesto | 2026-07-17 | edited | REWRITTEN end-to-end (knowledge-transfer thesis, directive R2-D1); supersedes round-1 golden fail; needs Ford's read; title strip |
| docs/10-system-design/00-interaction-surfaces | 2026-07-17 | edited | NEW — authored fresh (R2-D1: dual interaction surfaces); needs Ford's read. R2-D3: canvas embed header framing removed (diagram sits bare; hover controls). R2-D4: section previews now content-fitted — no frame rect/label chip, 16px world padding, natural image height (letterbox removed); needs new `fit=content` support in the CANVAS repo (uncommitted there). R2-D10: repaired drop-nesting staircase under "The Solution" (5 blocks un-nested to top level; editor drop-clamp fix landed) — awaiting Ford's visual check; title strip |
| docs/10-system-design/10-doc-architecture | 2026-07-17 | edited | NEW — authored fresh (R2-D1: structure decisions + rationale, absorbs framework standards' why); needs Ford's read; title strip |
...
| docs/10-system-design/30-block-vocabulary/10-rich-text/10-paragraph | | | vocab sweep; title strip |
| docs/10-system-design/30-block-vocabulary/10-rich-text/11-heading | | | vocab sweep; title strip |
| docs/10-system-design/30-block-vocabulary/10-rich-text/12-list-item | | | vocab sweep; title strip |
| docs/10-system-design/30-block-vocabulary/10-rich-text/13-quote | | | vocab sweep; title strip |
| docs/10-system-design/30-block-vocabulary/10-rich-text/14-callout | | | vocab sweep; title strip |
| docs/10-system-design/30-block-vocabulary/10-rich-text/15-divider | | | vocab sweep; title strip |
| docs/10-system-design/30-block-vocabulary/10-rich-text/16-image | | | vocab sweep; title strip |
| docs/10-system-design/30-block-vocabulary/10-rich-text/17-video | | | vocab sweep; title strip |
| docs/10-system-design/30-block-vocabulary/20-code | | | vocab sweep; title strip |
| docs/10-system-design/30-block-vocabulary/30-structured-table | | | vocab sweep; title strip |
| docs/10-system-design/30-block-vocabulary/40-file-tree | | | vocab sweep; title strip |
| docs/10-system-design/30-block-vocabulary/50-interaction-surface | | | vocab sweep; title strip |
| docs/10-system-design/30-block-vocabulary/60-mermaid | | | vocab sweep; title strip |
| docs/10-system-design/30-block-vocabulary/70-canvas | | | vocab sweep; title strip |
...
| docs/20-implementation/10-package-map | | | vocab sweep; title strip |
