...
describe("per-block-type layout overrides", () => {
  ...
  it("maps each named width onto the lane token it stands for", () => {
    ...
    expect(blockLayoutOverrideCss(withLayout({ code: { width: "wide" } }))).toContain(
      "max-width: var(--style-wide-width,1700px);",
    );
  ...
  it("persists into railDefaults, so locked serves and static exports see it", () => {
    // railDefaults is every setting EXCEPT `components`, so blockLayout rides
    // it with no new transport — the same path the other knobs use.
    const { components, ...railDefaults } = withLayout({ canvas: { justify: "center" } });
...
describe("style rail stock values match the consumers' inline fallbacks", () => {
  ...
  it("DocPage's left-margin fallback equals stock layout.contentMargin", () => {
    expect(DEFAULT_STYLE_RAIL_SETTINGS.layout.contentMargin).toBe(64);
    // 64px === 4rem at the 16px root font.
    expect(layoutSource).toContain("px-[var(--style-content-margin,4rem)]");
  });

  it("the wide lane's fallback equals stock layout.wideWidth", () => {
    expect(DEFAULT_STYLE_RAIL_SETTINGS.layout.wideWidth).toBe(1700);
    expect(laneSource).toContain("max-w-[var(--style-wide-width,1700px)]");
  });
...
describe("style rail wide lane", () => {
  it("defaults, clamps, and migrates the wide-lane width", () => {
    expect(DEFAULT_STYLE_RAIL_SETTINGS.layout.wideWidth).toBe(1700);
    expect(normalizeSettings({ accent: "purple" }).layout.wideWidth).toBe(1700);
    ...
    expect(normalizeSettings({ layout: { wideWidth: "wide" } }).layout.wideWidth).toBe(1700);
  });
  ...
  it("defaults and clamps the global left margin", () => {
    ...
    expect(DEFAULT_STYLE_RAIL_SETTINGS.layout.contentMargin).toBe(64);
    expect(normalizeSettings({ accent: "purple" }).layout.contentMargin).toBe(64);
...
describe("style rail repo baseline", () => {
  ...
  it("still emits vars for values sitting at the repo baseline", () => {
    ...
    // export would render 1700/64 instead of the repo's 2000/120.
