...
/**
 * This browser's effective settings: the stored blob layered OVER the repo
 * baseline (normalizeSettings fills from the baseline), or the bare
 * baseline when nothing is stored. Precedence, highest first:
 * localStorage -> themes/<id>/theme.json railDefaults -> stock.
 */
...
export function styleRailVars(settings: StyleRailSettings): Record<string, string | null> {
  ...
  const vars: Record<string, string | null> = {
    ...
    // grid, not to the body font's ch. Consumers carry the 1700px fallback
...
const BLOCK_LAYOUT_WIDTH_VALUES: Record<BlockLayoutWidth, string> = {
  ...
  wide: "var(--style-wide-width,1700px)",
  full: "none",
};
 ...
 * on the attribute pair both rendering surfaces already stamp
 * (`data-doc-lane` + `data-doc-block-type`).
 *
