...
export function StyleRailPane({
  ...
  const resetComponent = (file: string) => {
    ...
    // The pane's reset clears everything the pane owns, and since the Layout
    // section lives in this pane its lane override goes too — otherwise
    // "reset" would leave a width the user can still see on the page.
    const blockLayout = { ...settings.blockLayout };
    delete blockLayout[file];
    onSettingsChange({
      ...settings,
