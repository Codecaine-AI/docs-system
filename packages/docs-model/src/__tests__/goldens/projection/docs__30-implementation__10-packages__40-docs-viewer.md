...
## Image Grid Rendering and Editing

- Decision: The rich-text image-grid descriptor and editor node view share ImageGrid. The editor modifies typed props through updateAttributes and the existing operation diff.

- Why: One image renderer keeps asset resolution, ordering, and proportions consistent in readers, editor previews, static Docs exports, and published articles. CSS handles column wrapping without a client layout dependency.

- Applies to: image-grid.tsx, image-grid-editor-node-view.tsx, and image-grid.css.
