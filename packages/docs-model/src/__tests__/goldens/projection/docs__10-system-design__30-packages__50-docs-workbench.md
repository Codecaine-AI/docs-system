...
- `packages/docs-workbench/web/src/shell/App.tsx` owns the standalone navigation shell, and `packages/docs-workbench/web/src/pages/DocPage.tsx` owns its two modes: Edit is the default, always-editable auto-save surface; Annotate attaches comments to blocks and canvas objects. The page also surfaces live SSE change flashes and one-click undo, while `packages/docs-workbench/web/src/pages/BlocksPage.tsx` exposes the block library at `#/blocks`.
