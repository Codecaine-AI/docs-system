...
| alt | string | no | Alt text; the projection falls back to caption, then empty. |
| caption | string | no | Caption rendered under the image and as an italic line in the projection. |
...
- Always provide `alt`: the projection is text-first, and `![](path)` tells a reading agent nothing.
