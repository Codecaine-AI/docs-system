...
> **Decision: Fourteen types and no more** — The author surface stays deliberately minimal. The source of truth is `DOC_BLOCK_TYPES` in docs-model's `doc-schema.ts`: exactly fourteen type strings. A small vocabulary keeps the render stable, the editor learnable, and the agent edit surface enumerable.
...
- **Object (20s):** structured-table · file-tree · interaction-surface · sequence · canvas

- **Media (30s):** image · video

## Typed actions, in one paragraph

Every bundle supplies a manifest, closed TypeBox state schemas with a per-type `carriesText` fact, one file per action verb, and an `agent-view.ts` markdown render. Four bundles expose 13 typed actions — code (2), structured-table (5), file-tree (3), interaction-surface (3) — and canvas forwards 5 more to the external canvas authority. Actions ride the generic op kernel as `blockAction` ops; the kernel, hash preconditions, and undo are the mutation model's story, not this one. The per-type pages document each block's actions where they exist.
