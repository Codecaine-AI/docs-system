...
[
  ...
  { "insert": " to " },
  {
    ...
    "attributes": {
      "reference": {
        "kind": "doc",
        "path": "docs/10-system-design/10-block-vocabulary/00-overview.md",
...
Outbound, `delta-markdown.ts` renders spans to inline markdown for the runtime projection. Reference marks project as plain text (label, falling back to the reference path) rather than link syntax — the projection is a greppable terminal artifact, and a bare bracket-and-paren pair would be noise for `docs grep`.
...
The block that holds these spans is described in the document & block tree; which types may carry them is a fact owned by per-type block state; how spans project to greppable markdown is covered in canonical bytes.
