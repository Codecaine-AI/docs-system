...
| --- | --- | --- | --- |
| title | string | no | Label used in the agent surface and the rendered header. |
...
A labeled blockquote — `> **Mermaid[: <title>]** — <source>` — with the source flattened inline, deliberately *not* a ````mermaid` fence: the agent surface is a greppable summary line (`grep '> \*\*Mermaid'`), and the live render belongs to the viewer.
...
- Because the markdown render flattens the source onto one line, keep node labels self-explanatory — the markdown reader sees the raw Mermaid text.
