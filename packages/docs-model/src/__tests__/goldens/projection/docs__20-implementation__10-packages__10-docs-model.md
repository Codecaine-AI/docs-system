...
`packages/docs-model/src/components/validate.ts` compiles a static state-schema registry for the mutation path. That registry omits waterfall, so `insertBlock` and `updateBlock` do not have a compiled checker for waterfall props. For the types in the static registry, these operations validate the resulting props object, so unknown stored keys fail the closed schema even when an edit only touches text. Action parameter schemas are not required to be closed; local action dispatch ignores and discards stray parameters. `packages/docs-model/src/components/__tests__/schema-over-corpus.test.ts` applies every registered state schema to repository documents, while full-document validation remains a separate structural gate. This split keeps structural reads tolerant while registered mutation checks reject undeclared state.
