...
Whether a type carries delta text is a per-type fact, declared as `carriesText` in the component state definitions. Text means prose only for the rich-text flow types; for `code` and `sequence` the delta text is the source payload, and marks are not meaningful there.
...
| true | code, sequence | Source payload: the fenced code body or diagram source; spans are plain inserts. |
...
Family-wide agent guidance is the markdown bridge above; per-type agent notes live on each type page.
