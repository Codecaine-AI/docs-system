...
A bundle is a manifest (name, owned types, agent-facing description), a state definition per owned type, an action list, and an `agentView` render function. The bundle name doubles as the folder name, the discovery key, and the viewer mirror key.
...
| file-tree | no | entries: { path, note?, change?, from? }[] |
