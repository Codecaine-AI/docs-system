...
The why. Every structural rule's rationale lives in doc architecture; the intent behind the whole system lives in the manifesto. The skill states rules; the corpus defends them. When a structural decision changes, both update in the same change — the design doc carries the reasoning, the skill carries the new instruction.
