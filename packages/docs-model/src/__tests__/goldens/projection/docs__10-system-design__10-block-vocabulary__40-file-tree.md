...
The annotated path tree of the block vocabulary: a flat list of path entries in props that renders — and projects — as a `tree`-command drawing, with notes and per-entry change markers for describing repo slices and refactors.
...
- The rename marker is the whole diff story: set `change: "renamed"` plus `from` and the projection draws `old -> new` on one line.
