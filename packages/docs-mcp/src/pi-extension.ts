...
async function connect(config: PiLaunchConfig, cwd: string): Promise<BridgeConnection> {
  ...
  const client = new Client({ name: "codecaine-pi", version: "0.1.0" });
  try {
  ...
  return (pi: PiExtensionAPI) => {
    ...
    pi.on("session_start", async (_event, ctx) => {
      ...
      try {
          ...
          for (const tool of page.tools) {
            pi.registerTool({
              ...
              async execute(_id, params, signal) {
                if (!connection) throw new Error("Codecaine Docs is disconnected. Run /reload to reconnect.");
        ...
        await close();
        ctx.ui?.notify(`Codecaine Docs connection failed: ${error instanceof Error ? error.message : String(error)}`, "error");
        throw error;
      }
    });
