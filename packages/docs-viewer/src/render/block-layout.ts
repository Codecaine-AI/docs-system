 ...
 * text measure, left-justified. Both rendering surfaces read the SAME
 * declaration:
 *
 * - the read/annotate surface: DocBlockRenderer wraps each top-level block in
 *   its lane element, and
 * - the edit surface: editor/views/node-views.tsx's AtomBlockView puts the
 *   same classes on the atom node's NodeViewWrapper.
 *
 ...
 *   (the style rail's "Wide lane" knob, default 1700px). Tables, state shapes,
 ...
 * - `center` — the blog-post convention for media (images, video, canvas and
 *   sequence embeds): text stays left-justified, media centers on the page.
 */
...
/**
 * Lane width tokens. Each carries its own inline fallback because the
 * `--style-*` family is emitted only by the style rail (StyleRail.tsx's
 * `styleRailVars`, applied inline on `<html>`) and deliberately has no
 * semantic.css entries — a host without the rail still gets sane widths.
 */
const LANE_WIDTH_CLASSES: Record<DocBlockLaneWidth, string> = {
  text: "max-w-[var(--style-content-width,100ch)]",
  wide: "max-w-[var(--style-wide-width,1700px)]",
  full: "max-w-none",
};
 ...
 * element wears. Called by BOTH rendering surfaces so read and edit mode
 * place a given block type identically.
 */
...
/**
 * The `data-doc-lane` value stamped on every lane element. Read surface and
 * edit surface both stamp it, which is what lets the edit surface's
 * text-measure CSS fallback skip blocks that already carry a lane (see
 * `.docs-editor-prosemirror > *:not([data-doc-lane])` in the
 * workbench's index.css) and what makes lanes assertable in tests.
 */
/**
 ...
 * block type), on both surfaces. That pair is the override hook: a theme or a
 * host stylesheet can retune one block type's width or justification without
 * touching this module or the block's component, e.g.
 *
...
/**
 * The wide lane, left-justified — the shared declaration for the data-heavy
 * block types (structured-table, interaction-surface, state-shape,
 * process-outline). Exported so those four descriptors declare the SAME
 * object rather than four copies that can drift apart.
 */
export const WIDE_DATA_BLOCK_LAYOUT: DocBlockLayout = { width: "wide", justify: "left" };

/**
 * The wide lane, centered — the shared declaration for media (image, video,
 * canvas, sequence). Blog-post convention: prose reads down the left rail,
 * media centers on the page.
 */
export const CENTERED_MEDIA_BLOCK_LAYOUT: DocBlockLayout = { width: "wide", justify: "center" };
