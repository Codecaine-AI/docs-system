...
describe("DocBlockRenderer", () => {
  ...
  it("wraps each top-level block in its declared page lane, and leaves nested blocks unlaned", () => {
    ...
    expect(stateShapeLane?.className).toContain("max-w-[var(--style-wide-width,1700px)]");

    // ...and media centers on the page.
    const canvasLane = [...topLevel].find((lane) =>
      lane.querySelector('[data-doc-block="canvas"]'),
    );
    expect(canvasLane?.getAttribute("data-doc-lane")).toBe("wide-center");
    expect(canvasLane?.className).toContain("mx-auto");
