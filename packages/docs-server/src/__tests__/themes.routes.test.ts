...
describe("theme folder routes", () => {
  ...
  test("a theme-locked serve still READS railDefaults but cannot write them", async () => {
    ...
    await writeFile(
      join(dir, "theme.json"),
      JSON.stringify({ name: "Default", railDefaults: { layout: { contentMargin: 120 } } }),
    );
    ...
    expect(read.status).toBe(200);
    const body = (await read.json()) as { theme: { manifest: { railDefaults: unknown } } };
    expect(body.theme.manifest.railDefaults).toEqual({ layout: { contentMargin: 120 } });
    ...
    const refused = await locked.handle(
      new Request("http://localhost/api/themes", {
        ...
        body: JSON.stringify({
          id: "default",
          manifest: { name: "Default", railDefaults: { layout: { contentMargin: 8 } } },
        }),
      }),
    );
    ...
    expect(stillOnDisk.railDefaults).toEqual({ layout: { contentMargin: 120 } });
  });
